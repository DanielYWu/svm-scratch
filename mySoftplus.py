...
class mySoftplus ():
    """ class mySoftplus which implements SVM using
        Stochastic Gradient Descent implemneted with
        the using the Softplus function as replacement for the hinge loss.
    ...
    def optimize(self, X, y, _lambda, k):
        ...
        # start with our initial weight vector with ||w|| = 1 / _lambda
        w_current = i_weight
        ite = 0
        for t in range(1, 100 * X.shape[0]):
            ...
            # calculate our learning rate
            const_t = 1.0 / _lambda / (t)

            weight = np.zeros(X_t.shape[1])
            for i in range(X_t.shape[0]):
                # print(1 + np.exp(np.dot(X_t[i, :], w_current) * y_t[i]) - 1)
                weight += const_t / k * \
                    y_t[i] * X_t[i, :] / \
                    (1 + np.exp(np.dot(X_t[i, :], w_current) * y_t[i] - 1))

            weight += (1 - const_t * _lambda) * w_current
    ...
    def predict(self, X, y):
        ...
        """
        prediction = np.dot(self.w[:, None].T, X.T)
...
if __name__ == "__main__":
    ...
    for i in range(numruns):
        ...
        print("\n")
    # Print combined error rates for each train set percent averaged by the number of folds that ran
    print("------FINAL RESULT -------")
